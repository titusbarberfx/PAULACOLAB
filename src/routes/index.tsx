...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Paula Martínez · Creadora de contenido & lifestyle" },
      {
        name: "description",
        content:
          "Web oficial de Paula Martínez. Descubre su universo, conecta por Instagram, TikTok o WhatsApp y colabora con ella.",
      },
      { property: "og:title", content: "Paula Martínez · Creadora de contenido" },
      { property: "og:description", content: "Lifestyle, moda y colaboraciones. Conecta con Paula." },
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 backdrop-blur-md bg-background/70 border-b border-border/60">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <a href="#contacto" className="hidden md:inline-flex btn-3d bg-primary text-primary-foreground text-sm" style={{ padding: "0.6rem 1.1rem" }}>
          Colaborar
        </a>
      </div>
    </header>
      ...
      <div className="mx-auto max-w-5xl px-6">
        ...
        <div className="grid gap-12 md:grid-cols-5 md:items-center">
          ...
          <div className="md:col-span-3 space-y-5 text-lg leading-relaxed text-muted-foreground">
            <p>
              Soy <span className="font-semibold text-foreground">Paula Martínez</span>, creadora de contenido
              apasionada por la moda, los viajes y todo lo que inspira a vivir con intención.
            </p>
            <p>
            ...
            <div className="flex flex-wrap gap-2 pt-4">
              {["Moda", "Lifestyle", "Viajes", "Slow living", "Editorial"].map((t) => (
                <span key={t} className="rounded-full border border-border bg-card px-4 py-1.5 text-sm text-foreground">
                  {t}
                </span>
              ))}
            </div>
    ...
    <section id="galeria" className="py-24 md:py-32 bg-secondary/40">
      <div className="mx-auto max-w-6xl px-6">
        <div className="mb-12 flex items-end justify-between gap-6">
          ...
          <a href={INSTAGRAM_URL} target="_blank" rel="noreferrer" className="hidden text-sm font-medium text-muted-foreground hover:text-foreground md:inline-flex items-center gap-2">
    ...
    <section id="contacto" className="py-24 md:py-32">
      <div className="mx-auto max-w-5xl px-6">
        ...
        <div className="mt-12 flex flex-wrap items-center justify-center gap-6 text-sm text-muted-foreground">
          <span className="inline-flex items-center gap-2"><MapPin className="h-4 w-4" aria-hidden /><span>Barcelona · España</span></span>
          <span className="inline-flex items-center gap-2"><Mail className="h-4 w-4" aria-hidden /><span>colaboraciones@paulamartinez.com</span></span>
        </div>
      </div>
    </section>
  );
